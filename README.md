...
- **Best MCP tool-use benchmarks to track:** [MCPMark Verified](https://github.com/eval-sys/mcpmark) for verified state-changing workflows, [MCP-Atlas](https://github.com/scaleapi/mcp-atlas) for cross-server tool discovery and answer synthesis with distractor tools
...
- **Best realtime voice-model leaderboard:** [Artificial Analysis Speech to Speech Index](https://artificialanalysis.ai/speech-to-speech) for reasoning, grounded task completion, and human preference; inspect conversational dynamics, latency, and cost separately
...
- **Best agent security, governance, and red-team starting points:** [OWASP Top 10 for Agentic Applications (2026)](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/), [Agent Governance Toolkit](https://github.com/microsoft/agent-governance-toolkit), [AgentDojo](https://github.com/ethz-spylab/agentdojo), [OS-Harm](https://github.com/tml-epfl/os-harm), [DecodingTrust-Agent](https://github.com/AI-secure/DecodingTrust-Agent), [MCP Security Bench](https://github.com/dongsenzhang/MSB), [AgentDoG](https://github.com/AI45Lab/AgentDoG), [Snyk Agent Scan](https://github.com/snyk/agent-scan), [DeepTeam](https://github.com/confident-ai/deepteam), [Promptfoo](https://github.com/promptfoo/promptfoo), [PyRIT](https://github.com/microsoft/PyRIT), [garak](https://github.com/NVIDIA/garak)
...
- **Why it stands out:** Apache-2.0 local PDF parser that pairs deterministic CPU extraction with optional local hybrid OCR/VLM handling, while preserving element-level bounding boxes and adding prompt-injection filtering and Tagged PDF generation.
- **Best for:** teams that need fast, auditable PDF-to-Markdown/JSON with source coordinates or accessibility tagging, and can accept a Java-backed, PDF-only stack.
- **Evidence:** 28k+ GitHub stars and active v2.5 releases through August 2026; its public 200-document benchmark publishes ground truth and per-engine outputs, but remains maintainer-run. Last checked: 2026-08-24.
...
- **Letta** - https://github.com/letta-ai/letta-code - stateful agent harness with agent-editable, Git-backed memory, skills, cross-session search, and scheduled work; choose it when memory belongs inside the runtime rather than behind an add-on API. The [former repository](https://github.com/letta-ai/letta) retains the retired V1 server on its `archive` branch. Cloud is the default; select the local backend explicitly for local agent state. Last checked: 2026-09-24.
...
- **Mem0** - https://github.com/mem0ai/mem0 - add-on memory layer for user, session, and agent context, with library, self-hosted server, and managed deployment paths. Its published LoCoMo, LongMemEval, and BEAM scores describe the managed platform, including proprietary optimizations absent from the open-source SDK; do not assume self-hosted score parity. Last checked: 2026-09-24.
...
- **ReMe** - https://github.com/agentscope-ai/ReMe - local-first memory kit that keeps durable state as editable Markdown, separates source sessions, daily notes, and long-term digests, and automates capture, indexing, consolidation, and recall across MCP, CLI, HTTP, Python, and native agent integrations; Apache-2.0, 3.4k+ GitHub stars, active v0.4 releases, and maintainer-published LongMemEval and BEAM evaluations. Last checked: 2026-09-07.
- **Hindsight** - https://github.com/vectorize-io/hindsight - MIT-licensed memory service with explicit retain, recall, and reflect operations, hybrid semantic/keyword/graph/temporal retrieval, and evidence-linked consolidation; useful when an existing agent needs structured memory without replacing its harness. Budget for database operations and LLM-backed ingestion/reflection, not just retrieval; its [benchmark tooling](https://github.com/vectorize-io/hindsight-benchmarks) is maintainer-run, not a neutral cross-vendor ranking. Last checked: 2026-09-24.
...
### LongMemEval-V2
- **Link:** https://github.com/xiaowu0162/LongMemEval-V2
- **Why it stands out:** moves agent-memory evaluation beyond chat recall by asking systems to recover operating knowledge from long histories of multimodal web-agent trajectories, including changing state, recurring workflows, local gotchas, and invalid premises.
- **Best for:** testing whether prior execution becomes reusable experience, with both answer accuracy and query latency visible.
- **Evidence:** Apache-2.0 public harness, data, baselines, leaderboard packaging, and 451 manually curated questions across web and enterprise domains, with up to 500 trajectories and 115M tokens per haystack. Last checked: 2026-09-07.
...
### MCP-Atlas
- **Link:** https://github.com/scaleapi/mcp-atlas
- **Why it stands out:** Scale's benchmark tests tool discovery among plausible distractors, cross-server orchestration, error recovery, and answer synthesis against real MCP servers rather than synthetic function signatures.
- **Best for:** evaluating broad tool-selection and information-gathering reliability; pair it with MCPMark Verified for programmatically verified state-changing work, since Atlas grades final-answer claims with an LLM judge.
- **Evidence:** MIT-licensed harness with version-pinned Docker environments and 500 public tasks across 36 servers; the [official leaderboard](https://labs.scale.com/leaderboard/mcp_atlas) also uses 500 held-out tasks. Pin the harness, judge, tool-call budget, and task split when comparing runs; local defaults and published leaderboard methodology differ. Last checked: 2026-09-15.
...
- **Scope note:** [Sierra's benchmark documentation](https://sierra.ai/blog/tau-voice-benchmarking-real-time-voice-agents-on-real-world-tasks) limits coverage to English with TTS-generated personas, gives the user simulator the agent's transcript, and does not score speech naturalness; do not treat it as multilingual or human-caller validation. Last checked: 2026-09-16.

### Artificial Analysis Speech to Speech Index
- **Link:** https://artificialanalysis.ai/speech-to-speech
- **Why it stands out:** independent leaderboard combining speech reasoning, grounded τ-voice task completion, human Arena preference, and Arena tool-call success, with separate conversational-dynamics, latency, and cost views.
- **Best for:** choosing a realtime speech-to-speech model or provider on task reliability and user experience; check Full Duplex Bench separately for interruptions and turn-taking, and distinguish Arena tool-call grading from τ-voice database-state verification.
- **Evidence:** the [v2.0 methodology](https://artificialanalysis.ai/methodology/speech-to-speech-benchmarking) equally weights Big Bench Audio, τ-voice, Arena Score, and Task Success Rate, requiring all four to rank; Full Duplex Bench is no longer an index component. Compare scores within the same index version. Last checked: 2026-09-16.
...
- **Why it stands out:** OSWorld-based safety benchmark that scores what computer-use agents actually do across deliberate misuse, prompt injection, and unintended misbehavior instead of judging only their text.
- **Best for:** teams testing whether browser and desktop agents remain safe while operating real applications without losing legitimate task capability.
- **Evidence:** NeurIPS 2025 Spotlight with an Apache-2.0 harness, 150 public tasks across browser, email, code, and other desktop apps, released trajectories and human annotations, and an automated judge validated against humans. Last checked: 2026-09-08.
